...
export function QuickCreateActivity({
  ...
  const activeFolders = useMemo(
    () =>
      folders.filter(
        (folder) => folder.kind === "list" && folder.archivedAtMs === undefined,
      ),
    [folders],
  );
    ...
    <section className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3">
      ...
      <div className="mt-1 flex gap-2">
        <input
          ...
          className="min-h-11 min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3"
      ...
      <div className="mt-2 grid grid-cols-[44px_1fr_1fr] gap-2">
        <input
          ...
          className="h-11 w-11 rounded-lg border border-slate-300 bg-white p-1"
        />
        <select
          aria-label="Life area"
          value={lifeAreaId}
          onChange={(event) => setLifeAreaId(event.target.value)}
          className="min-h-11 min-w-0 rounded-lg border border-slate-300 bg-white px-2 text-sm"
        >
          <option value="">No area</option>
          {areas
            .filter((area) => area.archivedAtMs === undefined)
            .map((area) => (
      ...
      {matches.length > 0 && (
        <div
          className="mt-3 rounded-lg border border-amber-200 bg-white p-2"
          role="group"
          aria-label="Same-name activities"
        >
          <p className="mb-2 text-sm font-semibold text-slate-800">
            Reuse existing or create a separate activity?
          </p>
            ...
            {matches.map((match) => (
              <button
                ...
                onClick={() => {
                  setName("");
                  setMatches([]);
                  onComplete(match, true);
                }}
                className="flex min-h-11 w-full items-center gap-2 rounded-lg border border-slate-200 px-3 text-left text-sm"
              >
                <span
                ...
                <span className="min-w-0 flex-1">
                  <strong className="block truncate">{match.name}</strong>
                  <span className="text-xs text-slate-500">
                    {areas.find((area) => area.id === match.lifeAreaId)?.name ||
                      "No area"}{" "}
                    ·{" "}
                    {folders.find((folder) => folder.id === match.folderId)
                      ?.name || "No folder"}
                  </span>
                </span>
          ...
          <button
            ...
            className="mt-2 min-h-11 w-full rounded-lg border border-indigo-200 font-semibold text-indigo-700"
          >
            Create separate “{name.trim()}”
          </button>
        </div>
      )}
      <details className="mt-2 text-sm text-slate-600">
        ...
        <p className="pb-2 text-xs">
          Duration, count-up, scheduling, tags and categories remain available
          when the activity is added to a Session or Daily list.
        </p>
      </details>
    </section>
  );
